...
import { DistributorDto, DistributorForCreationDto } from 'src/app/_interface/distributor';
...
@Component({
  ...
  encapsulation: ViewEncapsulation.None // Để CSS có thể áp dụng cho dialog
})
export class DistributorsComponent implements OnInit, AfterViewInit {
  displayedColumns: string[] = ['action', 'distributorCode', 'distributorName', 'address', 'phoneNumber', 'contactSource', 'area', 'note'];
  ...
  addDistributor() {
    const popup = this.dialog.open(AddDistributorComponent, {
      width: '500px',
      height: '570px',
  ...
  updateDistributor(id: number) {
    const popup = this.dialog.open(UpdateDistributorComponent, {
      width: '500px',
      height: '567px',
      enterAnimationDuration: '100ms',
      exitAnimationDuration: '100ms',
  ...
  deleteDistributor(id: number) {
    this.dialogServe.openConfirmDialog('Are you sure, you want to delete this distributor?')
      .afterClosed()
      .subscribe((res) => {
        if (res) {
          ...
          this.repoService.delete(deleteUri).subscribe(
            (res) => {
              this.dialogServe.openSuccessDialog("The distributor has been deleted successfully.")
                .afterClosed()
                .subscribe(() => {
                  this.getDistributors();
                });
  ...
  this.repoService.upload('api/distributors/import', formData).subscribe(
    (res) => {
      this.dialogServe.openSuccessDialog('Distributors imported successfully.')
        .afterClosed()
        .subscribe(() => {
          this.getDistributors();
        });
    ...
    (error) => {
      this.dialogServe.openErrorDialog(`Error importing distributors: ${error.message}`);
    }
  );
